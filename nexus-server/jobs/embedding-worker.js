...
async function processQueue() {
  ...
  for (const [index, filePath] of batch.entries()) {
    ...
    // Pre-create the record as "processing" before attempting to load/embed
    // it, mirroring Document.addDocuments' non-native path - otherwise a
    // failure here leaves zero DB trace and the doc silently disappears from
    // both the KB UI and the /system/documents/failed retry queue instead of
    // showing up as "failed".
    let docRecord = null;
    try {
      docRecord = await prisma.workspace_documents.create({
        data: { docId, filename, docpath: filePath, workspaceId, status: "processing" },
      });
    } catch (err) {
      console.error("Failed to pre-create document record", err.message);
    }
      ...
      if (docRecord) {
        await prisma.workspace_documents
          .update({
            ...
            data: { status: "failed", statusMessage: "Failed to load file data" },
          })
    ...
    if (!vectorized) {
      ...
      if (docRecord) {
        await prisma.workspace_documents
          .update({
            where: { id: docRecord.id },
            data: { status: "failed", statusMessage: error || "Unknown error" },
          })
          .catch((err) => console.error(err.message));
      }
      emit({
    ...
    try {
      if (docRecord) {
        await prisma.workspace_documents.update({
          where: { id: docRecord.id },
          data: { metadata: newDoc.metadata, status: "completed" },
        });
      } else {
