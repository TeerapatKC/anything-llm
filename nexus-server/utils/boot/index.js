...
function bootHTTP(app, port = 3001) {
  ...
  app
    .listen(port, async () => {
      await markOnboarded();
      // Must run before anything that might read a user's role/permissions -
      // seeds/reconciles the permission catalog and built-in roles so the
      // instance is never left with an admin role that grants nothing.
      await Role.seed();
      await WorkspaceRole.seed();
      await setupTelemetry();
      new CommunicationKey(true);
