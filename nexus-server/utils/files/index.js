...
// The collector's hotdir is NOT derived from its own STORAGE_DIR (see
// collector/utils/constants.js - WATCH_DIRECTORY is always
// `<collector install dir>/hotdir`, fixed, regardless of STORAGE_DIR/NODE_ENV).
// The fallback below only lands on the right path if STORAGE_DIR happens to
// sit exactly two directories below wherever collector/ is installed (true
// for the local dev layout, where storage lives inside server/) - it breaks
// silently (ENOENT on upload) for any production deployment that points
// STORAGE_DIR at a separate data directory outside the app tree, which is
// exactly what this project's own deploy docs recommend
// (e.g. STORAGE_DIR=/var/lib/nexus-ai/storage with collector installed
// elsewhere, like /opt/nexus-ai/collector). HOTDIR_PATH lets a deployment
// state the real path explicitly instead of relying on that coincidence.
const hotdirPath = process.env.HOTDIR_PATH
  ? path.resolve(process.env.HOTDIR_PATH)
  : process.env.NODE_ENV === "development"
