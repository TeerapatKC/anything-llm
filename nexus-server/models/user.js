...
const User = {
  ...
  validations: {
    ...
    // Format-only check - role names are no longer a fixed enum (an operator
    // can create custom instance-wide roles via the Role model/`/roles/new`).
    // Whether `role` actually exists is checked separately with
    // `Role.exists()` wherever it's written (create/update below), since
    // that's an async DB/cache lookup and this validator is called
    // synchronously.
    role: (role = "default") => {
      const value = String(role || "default").trim().toLowerCase();
      if (!value) throw new Error("Role name cannot be empty.");
      return value;
    },
  ...
  create: async function ({
    ...
    try {
      ...
      const validatedRole = this.validations.role(role);
  ...
  update: async function (userId, updates = {}) {
    try {
      ...
      // `validations.role` above only checks the string's format - actual
      // existence is a DB/cache lookup, done here since it's async.
      ...
      if (updates.hasOwnProperty("password")) {
        ...
        // A successful password change satisfies the first-login reset
        // requirement, regardless of whether the caller explicitly asked to
        // clear it. `mustResetPassword` is intentionally NOT in `writable`
        // (so it can't be set/unset directly via generic updates) - this is
        // the one server-controlled place it's ever cleared.
