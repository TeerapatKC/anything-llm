...
function systemEndpoints(app) {
    ...
    async (request, response) => {
      try {
        ...
        if (!Array.isArray(names) || names.length === 0) {
          response
            .status(400)
            .json({ success: false, error: "names must be a non-empty array." });
          return;
        }
  ...
  // Compatibility shape for a newer upstream frontend branch
  // (feat/shadcn-migration) whose onboarding step checks/creates the first
  // admin through these paths instead of /system/enable-multi-user. Backed
  // by the exact same underlying flow - this just also mints a session token
  // in the response so onboarding can continue straight into the app instead
  // of bouncing to /login, which is what that frontend's UserSetup step
  // expects.
  app.get("/system/setup-state", async (_, response) => {
    try {
      const multiUserMode = await SystemSettings.isMultiUserMode();
      response.status(200).json({ needsAdminSetup: !multiUserMode });
    } catch (e) {
      console.error(e.message, e);
      response.sendStatus(500).end();
    }
  ...
  app.post("/system/setup-admin", async (request, response) => {
    try {
      if (await SystemSettings.isMultiUserMode()) {
        response.status(400).json({
          success: false,
          error: "An admin has already been set up for this instance.",
        });
        return;
      }
      ...
      if (error || !user) {
        response
          .status(400)
          .json({ success: false, error: error || "Failed to create admin account." });
        return;
      }

      await SystemSettings._updateSettings({ multi_user_mode: true });
      await updateENV({ JWTSecret: process.env.JWT_SECRET || v4() }, true);
      await Telemetry.sendTelemetry("enabled_multi_user_mode", { multiUserMode: true });
      await EventLogs.logEvent("multi_user_mode_enabled", {}, user.id);

      const { Role } = require("../models/role");
      const permissions = await Role.permissionsForUser(user);
      const token = makeJWT({ id: user.id, username: user.username }, process.env.JWT_EXPIRY);
      response.status(200).json({
        success: true,
  ...
  app.post("/system/user", [validatedRequest], async (request, response) => {
    try {
      ...
      // NOTE: must check the field was actually sent before comparing - a
      // request that omits `username` entirely (e.g. only updating bio)
      // would otherwise destructure to `undefined`, which is never equal to
      // sessionUser.username, and String(undefined) ("undefined") passes the
      // username validator - silently overwriting the real username.
      if (
        Object.prototype.hasOwnProperty.call(reqBody(request), "username") &&
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        // happens to have created - those are only editable through the
        // workspace-scoped route, which enforces workspace-admin permission
        // (this route only checks the caller is logged in at all).
