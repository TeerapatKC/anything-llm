...
class Provider {
  ...
  static contextLimit(provider = "openai", modelName) {
    ...
    if (!llm || !llm.hasOwnProperty("promptWindowLimit")) {
      console.warn(
        `\x1b[33m[.contextLimit warning]\x1b[0m Could not determine .promptWindowLimit for provider ${provider}. This could lead to incorrect context window management by NexusAI since we cannot determine the context window limit for this provider/model combination.`
      );
      return 8_000;
    }
