...
/**
 * Account-linking codes for the shared Telegram bot.
 *
 * The code is minted in the web app by a signed-in user, so holding one already
 * proves control of that account; typing it into Telegram is what proves control
 * of the chat. Codes live in memory only - they are short-lived, single-use, and
 * a restart losing them costs nothing more than asking for a new one.
 *
...
/** Attempts one Telegram chat may make before it is told to wait. */
 ...
 */
function checkAttemptAllowance(chatId) {
  const now = Date.now();
  const entry = attempts.get(String(chatId));
  if (!entry) return { allowed: true, retryInSeconds: 0 };

  if (now - entry.windowStartedAt >= ATTEMPT_WINDOW_MS) {
    attempts.delete(String(chatId));
