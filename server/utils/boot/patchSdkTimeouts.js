 ...
 *
 * When `NEXUSAI_FETCH_TIMEOUT` is set (milliseconds), both the undici
...
function patchSdkTimeouts() {
  const envDefinedTimeout = process.env.NEXUSAI_FETCH_TIMEOUT;
  const envDefinedMaxRetries = process.env.NEXUSAI_MAX_RETRIES;
  ...
  if (envDefinedTimeout) {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0) {
      console.warn(
        `${LOG_PREFIX} NEXUSAI_FETCH_TIMEOUT="${envDefinedTimeout}" is not a valid positive integer — using default ${DEFAULT_TIMEOUT_MS}ms.`
      );
    } else {
      timeoutMs = parsed;
    }
  ...
  if (envDefinedMaxRetries) {
    ...
    if (!Number.isFinite(parsed) || parsed < 0) {
      console.warn(
        `${LOG_PREFIX} NEXUSAI_MAX_RETRIES="${envDefinedMaxRetries}" is not a valid non-negative integer — using default ${DEFAULT_MAX_RETRIES}.`
      );
    } else {
      maxRetries = parsed;
    }
