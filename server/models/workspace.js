...
const Workspace = {
  ...
  update: async function (id = null, updates = {}) {
    ...
    // When switching to nexusai-router, chatModel is not used.
    // When switching away from nexusai-router, clear router_id.
    if (validatedUpdates?.chatProvider === "nexusai-router") {
      validatedUpdates.chatModel = null;
    } else if (
      validatedUpdates?.chatProvider &&
      validatedUpdates.chatProvider !== "nexusai-router"
    ) {
