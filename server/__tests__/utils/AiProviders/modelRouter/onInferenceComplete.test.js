...
function makeRouter() {
  const router = new NexusAIModelRouter({ slug: "test-ws", name: "Test" });
  router._routeKey = "user:1|test-ws|thread:1";
  router.resolvedRoute = { ...NON_FALLBACK_ROUTE };
  return router;
}

describe("NexusAIModelRouter.onInferenceComplete", () => {
...
describe("NexusAIModelRouter delegate instrumentation", () => {
